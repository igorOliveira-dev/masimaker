...
`}</style>
      ...
      <header className="w-full bg-(--background-secondary)/50 backdrop-blur-sm fixed flex items-center justify-between px-6 py-5 md:px-12 z-1">
        <span className="text-(--purple) font-bold tracking-tight [font-family:var(--font-press-start-2p)]">
          Masi Maker
        </span>
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/auth/login"
            className="bg-(--purple) rounded-lg border border-(--purple)/50 text-white text-[13px] font-medium [font-family:var(--font-press-start-2p)] px-3 py-2 transition-transform hover:bg-(--purple)/80 focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-[#711b9f]"
          >
            Log in
          </Link>
        </div>
      ...
      <section className="mt-16 mx-auto grid max-w-6xl gap-12 px-6 pb-20 pt-8 md:grid-cols-2 md:items-center md:px-12 md:pt-16">
        <div>
          <span className="mm-block inline-block rounded-full border border-(--purple)/50 bg-(--purple)/90 px-3 py-1 text-xs text-white [font-family:var(--font-press-start-2p)] [animation-delay:0.04s]">
          ...
          <p className="mm-block mt-5 max-w-md text-base leading-relaxed [var(--foreground)] [animation-delay:0.08s] md:text-lg font-roboto font-semibold">
            Masi Maker is a visual editor for creating landing pages without writing a single line of code. Choose a
            template, build your sections, and publish — all by dragging ready-made components.
          </p>

          <div className="mm-block mt-8 flex flex-wrap gap-3 [animation-delay:0.16s]">
            <Link
              href="/auth/register"
              className="rounded-lg bg-(--purple) px-5 py-3 font-semibold [font-family:var(--font-press-start-2p)] text-white transition-transform hover:-translate-y-0.5 hover:bg-[#71129f]/80  focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-[#711b9f]"
            >
              Start creating
            </Link>
            <Link
              href="/dashboard"
              className="rounded-lg border border-(--foreground)/50 px-5 py-3 text-foreground font-semibold [font-family:var(--font-press-start-2p)] transition-colors hover:-translate-y-0.5 hover:bg-(--foreground)/20 focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-[#5B4FE8]"
            >
        ...
        <div className="mm-block mx-auto w-full max-w-md rounded-xl border border-[#DEDAD0] bg-white shadow-sm [animation-delay:0.1s]">
          ...
          <div className="space-y-3 p-4">
            ...
            <div className="mm-block space-y-2 rounded-lg bg-[#F6F4EF] p-3 [animation-delay:0.22s]">
              <div className="h-3 w-2/3 rounded bg-(--purple)" />
            ...
            <div className="mm-block grid grid-cols-3 gap-2 rounded-lg bg-[#F6F4EF] p-3 [animation-delay:0.3s]">
              ...
              <div className="space-y-1.5">
                <div className="h-10 rounded bg-(--purple)" />
                <div className="h-1.5 w-3/4 rounded bg-[#DEDAD0]" />
              </div>
            </div>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        ...
        <p className="mt-2 max-w-lg text-foreground font-roboto font-semibold">
          Three simple layers, one inside the other — like a Russian nesting doll of blocks.
        </p>

        <div className="mt-8 rounded-2xl border border-(--foreground)/10 p-2">
          {structure.map((item, i) => (
            <div
              key={item.tag}
              className={`rounded-xl p-4 md:p-6 ${
                i % 2 === 0 ? "bg-(--background-tertiary)" : "bg-(--background-secondary)"
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          {templates.map((t) => (
            <div
              key={t.label}
              className="rounded-xl border border-(--foreground)/5 bg-(--background-secondary) p-5 transition-transform hover:bg-[#3A3B54]/20 hover:-translate-y-1"
            >
              <div className={`h-16 rounded-lg ${t.tintSoft}`}>
                <div className={`h-full w-1/3 rounded-l-lg ${t.tint}`} />
              </div>
              <p className="mt-4 font-bold [font-family:var(--font-press-start-2p)]">{t.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-12">
        <div className="flex flex-col items-start gap-6 rounded-2xl bg-(--background-secondary) px-6 py-10 md:flex-row md:items-center md:justify-between md:px-10">
          <div>
            <h2 className="text-1xl font-bold tracking-tight text-foreground [font-family:var(--font-press-start-2p)]">
          ...
          <div className="flex shrink-0 gap-3">
            <Link
              href="/dashboard"
              className=" h-12 rounded-lg bg-(--purple) px-5 py-4 text-[10px] [font-family:var(--font-press-start-2p)] font-semibold text-white transition-transform hover:bg-(--purple)/80 hover:-translate-y-0.5 focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-white"
            >
              Create my page
            </Link>
            <Link
              href="/auth/login"
              className="h-12 rounded-lg border border-(--foreground)/30 px-5 py-4 text-[10px] font-semibold [font-family:var(--font-press-start-2p)] text-foreground focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-white hover:bg-(--foreground)/20 transition-transform hover:-translate-y-0.5"
            >
              Log in
            </Link>
          </div>
      ...
      <footer className="border-t border-(--background-tertiary)/80 px-6 py-8 text-xs text-foreground md:px-12 font-roboto">
